...
if run_method == 'panair':
	# Run (Panair)
	axiebump = AxieBump(CASE_DIR, PANAIR_EXE, SBOOM_EXE, altitude=alt_ft, deformation=deformation)
	...
	axiebump = EquivArea(CASE_DIR, SBOOM_EXE, altitude=alt_ft, deformation=deformation)
	loudness = axiebump.run(bump_inputs)
else:
