...
def parametrize_humidity(humidity,temperature,cutoff_altitude):
    ...
    saturation_vapor_pressures = [[t[0],0.61121*np.exp((18.678-(t[1]/234.5))*(t[1]/(257.14+t[1])))] for t in temperature]
    actual_vapor_pressures = [[new_humidity[i][0],new_humidity[i][1]/100*saturation_vapor_pressures[i][1]] for i in range(len(new_humidity))]
    ...
    new_humidity = [[0,equivalent_relative_humidity*100],[cutoff_altitude,0],[actual_vapor_pressures[-1][0],0]]
