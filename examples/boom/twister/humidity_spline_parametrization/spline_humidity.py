...
# sample_weights = sample_weights**4
sample_weights = -1/2*(sample_weights+0.2)**2+sample_weights+1
# break_point = int(np.round(len(profile_altitudes)/2))
# sample_weights[:break_point] = [0.5 for i in range(break_point)]
# sample_weights[break_point:] = [1.0 for i in range(break_point)]
# sample_weights = None
