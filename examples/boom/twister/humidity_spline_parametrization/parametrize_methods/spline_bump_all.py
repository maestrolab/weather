'''Script to run the two spline parametrization process for multiple profiles at
a time.''' 
...
stored_data = {}
stored_data['noise'] = []
latitudes = list(range(20,32))
longitudes = list(range(-110,-90))
...
for lat in latitudes:
    for lon in longitudes:
        ...
        sample_weights = initialize_sample_weights(profile_altitudes, type = 'bump')
        ...
        stored_data[lat,lon] = {'parameters':x, 'RMSE':p_profile.rmse, 'noise':noise}
        stored_data['noise'].append(noise['difference'])

filename = '%s_%s_%s_%sall.p' % (day, month, year, hour)
all_data = open(filename, 'wb')
pickle.dump(stored_data, all_data)
all_data.close()
