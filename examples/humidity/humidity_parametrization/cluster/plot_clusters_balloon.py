...
initial_profile = truncate_at_altitude(
    interpolated_profiles[0], cruise_altitude=model['cruise_altitude'])
truncated_profiles = np.array([initial_profile])

for profile in interpolated_profiles[1:]:
    truncated_profile = truncate_at_altitude(profile, cruise_altitude=model['cruise_altitude'])
...
for key in keys[:]:
    ...
    ax[row][col].plot(rh, alts, color='k', alpha=0.05)  # , color = 'k')
