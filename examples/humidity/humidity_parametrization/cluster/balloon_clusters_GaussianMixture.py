...
GMM.fit(mixture_vector)
y_GMM = GMM.fit_predict(mixture_vector)
...
for i in range(len(averages)):
    ...
    plt.scatter(averages[i], deviations[i], c=y_GMM)

# Plot clusters
data = [data_interpolated['rh'][y_GMM == i] for i in range(n_clusters)]
...
for j in range(n_clusters):
    ...
    data_i = data_interpolated['rh'][y_GMM == j]
...
data = [data_interpolated['vps'][y_GMM == i] for i in range(n_clusters)]
average_plot = np.array([[np.average(data[i][:,:,1], axis = 0)] for i in range(n_clusters)])

plt.figure()
for j in range(n_clusters):
    plt.subplot(2,2,j+1,xlabel='Vapor Pressure [kPa]',ylabel='Altitude [m]')
    data_i = data_interpolated['vps'][y_GMM == j]
    for profile in data_i:
        alts, vps = profile.T
